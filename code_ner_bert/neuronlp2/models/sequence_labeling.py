...
class BiRecurrentConv(nn.Module):
    ...
    def _get_rnn_output(self, input_word, input_char, mask=None, length=None, hx=None, feature=False):
        ...
        if feature:
            return output, hn, mask, length, word, char
...
class BiRecurrentConvCRF(BiRecurrentConv):
    ...
    def decode(self, input_word, input_char, target=None, mask=None, length=None, hx=None, leading_symbolic=0):
        ...
        preds, conf = self.crf.decode(output, mask=mask, leading_symbolic=leading_symbolic)
        
        if mask is None:
            return preds, torch.eq(preds, target.data).float().sum(), conf
        else:
            return preds, (torch.eq(preds, target.data).float() * mask.data).sum(), conf

    def feature(self, input_word, input_char, target=None, mask=None, length=None, hx=None, leading_symbolic=0):
        output, _, mask, length, word, char = self._get_rnn_output(input_word, input_char, mask=mask, length=length, hx=hx, feature=True)
        # print(output.size(), word.size(), char.size())
        return output

        # if target is None:
        #     return self.crf.decode(output, mask=mask, leading_symbolic=leading_symbolic), None, output, length
        ...
        # if mask is None:
        #     return preds, torch.eq(preds, target.data).float().sum(), output, length
        # else:
        #     return preds, (torch.eq(preds, target.data).float() * mask.data).sum(), output, length